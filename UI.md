# UI inventory

## Public

Homepage: sticky navigation, academy hero, offers, featured program, academy reasons, learning journey, founder, genuine trust, resources, final CTA and footer.

Course landing preserves the cream/navy editorial language with a strong navy hero, curriculum sequence, outcomes, inclusions, FAQ and final CTA. eBook uses real contents previews. Workshop and verification remain focused flows.

## Student

Learning dashboard surfaces active/preview status, next live class, progress, modules, recordings, files, Calendar/Meet, community and certificate. Locked resources remain visually previewed but are also blocked server-side/RLS.

## Admin

Combined command center follows the selected mock: grouped navy sidebar, KPI band, action queue, first-party funnel, website status, next class, recent purchase and integration health. Website Studio offers draft, preview and publish. Other operational modules share search, empty/loading and creation patterns.

## Responsive/accessibility

- 390px and 1440px are core visual QA sizes
- Mobile navigation/admin sidebar use accessible sheets
- Visible focus, medium-speed press states, reduced-motion support
- No horizontal overflow; Bengali wrapping and touch targets are tested
- Genuine empty states are explicit; production never shows local demo metrics
